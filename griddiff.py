    ...
    with open('point-eov-eht2-3.0-hun.txt', 'r') as point_eov_eht2_3_0_txtfile:
      with open('grid-wgs-gc-eov-eht2-3.0-hun.txt', 'w') as grid_wgs_gc_eov_eht2_3_0_txtfile:
        ...
        for wgsline in point_wgs_gc_txtfile:
          ...
          grid_wgs_gc_eov_eht2_3_0_txtfile.write('%s %s %s %s %s %s\r\n' % (wgsX, wgsY, wgsZ, diffX, diffY, diffZ))
          if mindiffX < diffX:
            mindiffX = diffX
          if maxdiffX > diffX:
            maxdiffX = diffX
...
print ('Diffs minX/maxX: %s/%s minY/maxY: %s/%s minZ/maxZ: %s/%s' % (mindiffX, maxdiffX, mindiffY, maxdiffY, mindiffZ, maxdiffZ))