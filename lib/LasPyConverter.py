...
class LasPyConverter:
    ...
    def DumpHeaderFormat(self):
        for spec in self.__OpenedFile.header.header_format:
            ...
            logging.info('Setting: %s: %s' % (spec.name, in_spec))
    ...
    def SetScale(self):
        # Use scale as is as
        self.__Scale = [self.__OpenedFile.header.scale[0], self.__OpenedFile.header.scale[1],
                        self.__OpenedFile.header.scale[2]]
        # Transform offset
        self.__Offset = [self.__OpenedFile.header.offset[0], self.__OpenedFile.header.offset[1],
                         self.__OpenedFile.header.offset[2]]

    def ScaleDimension(self):
        self.__TransformedOffset = np.array([0, 0, 0])
        self.__TransformedOffset[0], self.__TransformedOffset[1], self.__TransformedOffset[2] = transform(
            self.__SourceProj, self.__DestinationProj, self.__Offset[0], self.__Offset[1], self.__Offset[2])
        self.__TransformedOffset = [math.floor(self.__TransformedOffset[0]), math.floor(self.__TransformedOffset[1]),
                                    math.floor(self.__TransformedOffset[2])]
        logging.info('LAS PointCloud file transformed offset: %s %s %s' % (
        self.__TransformedOffset[0], self.__TransformedOffset[1], self.__TransformedOffset[2]))
        logging.info('Updating LAS file header offsets.')
        self.__OpenedFile.header.set_offset(
            (self.__TransformedOffset[0], self.__TransformedOffset[1], self.__TransformedOffset[2]))
    ...
    def ReturnPointCloud(self):
        ...
        return np.array([self.__OpenedFile.X * self.__Scale[0] + self.__Offset[0],
                         self.__OpenedFile.Y * self.__Scale[1] + self.__Offset[1],
                         self.__OpenedFile.Z * self.__Scale[2] + self.__Offset[2],
                         self.__OpenedFile.intensity, self.__OpenedFile.flag_byte, self.__OpenedFile.raw_classification,
                         self.__OpenedFile.scan_angle_rank, self.__OpenedFile.user_data, self.__OpenedFile.pt_src_id,
